...
class MyOwnNetwork(ClassificationNet):
    ...
    def __init__(self, activation = "Relu", num_layer=2,
        ...
        ########################################################################
        self.str_activation = activation
        ...
        if flag == 1 :
            ...
            for i in range(num_layer-1):
                #print("num_layer:", i+1)
                in_size = input_size
                out_size = int(input_size/5)
                print(in_size, out_size)
                
                
                self.params['W' + str(i + 1)] = std * np.random.randn(in_size,
                                                                      out_size)
                self.params['b' + str(i + 1)] = np.zeros(out_size)
                
                #self.params = {'W' + str(i+1): std * np.random.randn(in_size, out_size),
                #       'b' + str(i+1): np.zeros(out_size)}
            ...
            self.params['W' + str(num_layer)] = std * np.random.randn(input_size,
                                                                     num_classes)
            self.params['b' + str(num_layer)] = np.zeros(num_classes)
            self.grads = {}
    ...
    def forward(self, X):
        ...
        for i in range(self.num_layer - 1):
            ...
            # Forward i_th layer
            X, cache_affine = affine_forward(X, W, b) #out, cache
            self.cache["affine" + str(i + 1)] = cache_affine

            # Activation function
            X, cache_activation = self.activation.forward(X)
            self.cache[self.str_activation + str(i + 1)] = cache_activation
